...
// ── Pick up flash messages set by update_profile.php ────────
$success = $_SESSION['profile_success'] ?? '';
$error   = $_SESSION['profile_error']   ?? '';
unset($_SESSION['profile_success'], $_SESSION['profile_error']);

// ── Load fresh student row from DB ──────────────────────────
$stmt = $pdo->prepare("SELECT * FROM students WHERE id = ? LIMIT 1");
$stmt->execute([$_SESSION['student_id']]);
$student = $stmt->fetch();

// Keep session photo in sync
...
// Helper: prefer DB row value, fall back to session
function val($student, $key, $session_key = null) {
    if ($student && isset($student[$key]) && $student[$key] !== '') {
...
:root {
  ...
  --white:     #ffffff;
  --radius:    8px;
  --shadow:    0 1px 4px rgba(0,58,107,0.08);
  --shadow-md: 0 4px 20px rgba(0,58,107,0.12);
  --red:       #dc2626;
  --red-lt:    #fef2f2;
  --green:     #16a34a;
  --green-lt:  #f0fdf4;
...
/* NAV */
nav { background: var(--blue-dk); height: 60px; padding: 0 28px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 100; border-bottom: 1px solid rgba(255,255,255,0.08); }
.nav-title { font-size: 17px; font-weight: 700; color: #fff; }
.nav-links { display: flex; align-items: center; gap: 2px; }
.nav-links a { font-size: 13px; font-weight: 500; color: rgba(255,255,255,0.78); text-decoration: none; padding: 6px 11px; border-radius: 6px; transition: all .15s; }
.nav-links a:hover { color: #fff; background: rgba(255,255,255,0.1); }
.nav-links a.active { color: #89CFF1; }
.nav-dropdown { position: relative; }
.nav-dropdown > a { display: flex; align-items: center; gap: 4px; }
.nav-dropdown > a .chevron { font-size: 10px; color: rgba(255,255,255,0.45); transition: transform .2s; }
...
.dropdown-menu a { display: block; padding: 9px 16px; font-size: 13px; color: var(--gray-600) !important; border-radius: 0 !important; background: transparent !important; }
.dropdown-menu a:hover { background: var(--gray-50) !important; color: var(--blue) !important; }
.btn-logout { background: #e53e3e; color: #fff !important; font-weight: 600 !important; border-radius: 6px; padding: 6px 16px !important; margin-left: 6px; }
.btn-logout:hover { background: #c53030 !important; }

/* PAGE */
.page-body { max-width: 920px; margin: 0 auto; padding: 36px 20px 70px; }
.page-header { text-align: center; margin-bottom: 28px; }
.page-header h1 { font-size: 22px; font-weight: 700; color: var(--blue-dk); }
.page-header p  { font-size: 13px; color: var(--gray-400); margin-top: 4px; }

/* ALERTS */
.alert { display: flex; align-items: center; gap: 10px; padding: 12px 16px; border-radius: var(--radius); font-size: 13.5px; font-weight: 500; margin-bottom: 22px; }
...
/* PHOTO CARD */
.photo-card { background: var(--white); border-radius: var(--radius); border: 1px solid var(--gray-200); box-shadow: var(--shadow-md); overflow: hidden; }
.card-head { background: var(--blue); padding: 13px 18px; }
.card-head h3 { color: #fff; font-size: 13px; font-weight: 600; }
.card-head p  { color: rgba(255,255,255,0.6); font-size: 12px; margin-top: 2px; }
...
.avatar-circle .u-icon svg { width: 52px; height: 52px; stroke: var(--blue); fill: none; stroke-width: 1.5; stroke-linecap: round; stroke-linejoin: round; }
...
.stu-name   { font-size: 15px; font-weight: 700; color: var(--blue-dk); text-align: center; line-height: 1.35; }
.stu-course { font-size: 12px; color: var(--gray-400); text-align: center; margin-top: 2px; }
.id-pill    { background: var(--blue-lt); border: 1px solid var(--blue-bd); border-radius: 20px; padding: 5px 14px; font-size: 12px; font-weight: 600; color: var(--blue); }
...
.uz-label { font-size: 12.5px; color: var(--gray-600); margin-top: 4px; font-weight: 500; }
...
.btn-upload { width: 100%; padding: 9px; border: none; border-radius: var(--radius); background: var(--blue); color: #fff; font-size: 13px; font-weight: 600; font-family: 'Inter', sans-serif; cursor: pointer; transition: background .15s; display: flex; align-items: center; justify-content: center; gap: 6px; }
...
/* FORM CARD */
.form-card { background: var(--white); border-radius: var(--radius); border: 1px solid var(--gray-200); box-shadow: var(--shadow-md); overflow: hidden; }
.form-body { padding: 26px 28px 30px; }
.section-label { font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: var(--blue); padding-bottom: 8px; border-bottom: 1px solid var(--gray-100); margin-bottom: 14px; margin-top: 22px; }
...
.field label { display: flex; align-items: center; gap: 5px; font-size: 11.5px; font-weight: 600; color: var(--gray-600); margin-bottom: 5px; }
...
.field input, .field select { width: 100%; padding: 9px 12px; border: 1.5px solid var(--gray-200); border-radius: var(--radius); font-size: 13.5px; font-family: 'Inter', sans-serif; color: var(--gray-800); background: var(--white); outline: none; transition: border-color .15s, box-shadow .15s; }
...
.btn-save { flex: 1; padding: 10px; border: none; border-radius: var(--radius); background: var(--blue); color: #fff; font-size: 14px; font-weight: 600; font-family: 'Inter', sans-serif; cursor: pointer; transition: background .15s; display: flex; align-items: center; justify-content: center; gap: 7px; }
...
.btn-cancel { padding: 10px 18px; border-radius: var(--radius); border: 1.5px solid var(--gray-200); background: transparent; color: var(--gray-600); font-size: 14px; font-weight: 500; font-family: 'Inter', sans-serif; cursor: pointer; transition: all .15s; white-space: nowrap; display: flex; align-items: center; gap: 6px; }
      ...
      <div class="photo-body">
        ...
        <div class="avatar-wrap">
          <div class="avatar-circle">
            <?php if ($photoSrc): ?>
              <img src="<?php echo $photoSrc; ?>" alt="Profile Photo" id="avatarImg"/>
            <?php else: ?>
              <div class="u-icon" id="avatarIcon">
        ...
        <div class="stu-name"><?php echo htmlspecialchars($_SESSION['fullname'] ?? 'Student'); ?></div>
        <div class="stu-course">
          <?php echo htmlspecialchars($_SESSION['course'] ?? ''); ?>
          &bull; Year <?php echo htmlspecialchars($_SESSION['year_level'] ?? ''); ?>
        </div>
        <div class="id-pill">ID: <?php echo htmlspecialchars($_SESSION['id_number'] ?? ''); ?></div>

        <!-- Photo upload — posts to update_profile.php -->
        <form method="POST" action="update_profile.php" enctype="multipart/form-data"
          ...
          <div class="upload-zone" id="uploadZone">
            ...
            <div class="uz-hint">JPG, PNG, WEBP &bull; Max 2 MB</div>
    ...
    <div class="form-card">
      ...
      <div class="form-body">
        ...
        <form method="POST" action="update_profile.php">
          ...
          <div class="field">
            <label>
              <svg viewBox="0 0 24 24"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="16" y1="10" x2="21" y2="10"/><line x1="16" y1="14" x2="21" y2="14"/><circle cx="8.5" cy="10" r="2.5"/><path d="M4 19c0-2.2 2-4 4.5-4s4.5 1.8 4.5 4"/></svg>
              ID Number
              <small style="font-weight:400;color:var(--gray-400);">(cannot be changed)</small>
            </label>
            <input type="text" value="<?php echo val($student,'id_number','id_number'); ?>" readonly/>
          </div>
          ...
          <div class="field-row3">
            <div class="field">
              ...
              <input type="text" name="lastname"
                     value="<?php echo val($student,'lastname'); ?>"
            ...
            <div class="field">
              ...
              <input type="text" name="firstname"
                     value="<?php echo val($student,'firstname'); ?>"
            ...
            <div class="field">
              ...
              <input type="text" name="middlename"
                     value="<?php echo val($student,'middlename'); ?>"
          ...
          <div class="field-row">
            <div class="field">
              ...
              <input type="email" name="email"
                     value="<?php echo val($student,'email'); ?>"
            ...
            <div class="field">
              ...
              <input type="text" name="address"
                     value="<?php echo val($student,'address'); ?>"
          ...
          <div class="field-row">
            ...
            <div class="field">
              ...
              <select name="year_level" required>
                <?php
                  $curY = (int)($student['year_level'] ?? $_SESSION['year_level'] ?? 1);
                  $yLabels = ['1st Year'=>1,'2nd Year'=>2,'3rd Year'=>3,'4th Year'=>4];
                  foreach ($yLabels as $lbl => $v) {
                    $sel = ($curY === $v) ? 'selected' : '';
          ...
          <div class="field-row">
            <div class="field">
              ...
              <div class="pw-wrap">
                ...
                <span class="pw-eye" onclick="togglePw('pw1','eye1')" title="Show / hide">
            ...
            <div class="field">
              ...
              <div class="pw-wrap">
                ...
                <span class="pw-eye" onclick="togglePw('pw2','eye2')" title="Show / hide">
