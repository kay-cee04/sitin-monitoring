...
// ── Always reload the latest student data from DB ────────────
// This ensures edits made in profile.php are reflected immediately
$stmt = $pdo->prepare("SELECT * FROM students WHERE id = ? LIMIT 1");
$stmt->execute([$_SESSION['student_id']]);
$student = $stmt->fetch();

if ($student) {
    // Sync every session key with the freshest DB values
...
/* LAYOUT */
.dashboard { max-width: 1200px; margin: 0 auto; padding: 24px 20px; display: grid; grid-template-columns: 240px 1fr 340px; gap: 20px; align-items: start; }
...
.card-head .card-icon { display: flex; align-items: center; }
.card-head .card-icon svg { width: 15px; height: 15px; stroke: rgba(255,255,255,0.8); fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }

/* STUDENT CARD */
.student-avatar { display: flex; flex-direction: column; align-items: center; padding: 20px 16px 16px; border-bottom: 1px solid var(--gray-100); }
.avatar-circle { width: 88px; height: 88px; border-radius: 50%; background: var(--blue-lt); border: 3px solid var(--blue-bd); display: flex; align-items: center; justify-content: center; color: var(--blue); box-shadow: 0 2px 8px rgba(0,58,107,0.15); overflow: hidden; }
.avatar-circle img { width: 100%; height: 100%; object-fit: cover; border-radius: 50%; }
.avatar-circle svg { width: 40px; height: 40px; stroke: var(--blue); fill: none; stroke-width: 1.5; stroke-linecap: round; stroke-linejoin: round; }
.student-info-list { padding: 14px 16px; }
.info-row { display: flex; align-items: flex-start; gap: 10px; padding: 7px 0; border-bottom: 1px solid var(--gray-100); font-size: 13px; }
.info-row:last-child { border-bottom: none; }
.info-icon { display: flex; align-items: center; justify-content: center; width: 18px; flex-shrink: 0; margin-top: 1px; }
.info-icon svg { width: 13px; height: 13px; stroke: var(--blue); fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
.info-label { font-weight: 600; color: var(--gray-600); margin-right: 4px; white-space: nowrap; }
.info-value  { color: var(--gray-800); }

/* ANNOUNCEMENTS */
.ann-scroll { max-height: 420px; overflow-y: auto; }
...
.ann-meta { font-size: 12.5px; font-weight: 600; color: var(--blue); margin-bottom: 8px; }
.ann-bubble { background: var(--gray-50); border: 1px solid var(--gray-100); border-radius: 6px; padding: 10px 12px; font-size: 13px; color: var(--gray-600); line-height: 1.6; }
.ann-empty { font-size: 13px; color: var(--gray-400); font-style: italic; }

/* RULES */
.rules-scroll { max-height: 420px; overflow-y: auto; padding: 16px 18px; }
...
.rules-header h3 { font-size: 15px; font-weight: 700; color: var(--blue-dk); }
.rules-header p  { font-size: 11.5px; font-weight: 600; color: var(--gray-600); margin-top: 3px; letter-spacing: 0.02em; }
.rules-section-title { font-size: 11.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--blue); margin: 14px 0 10px; }
...
.rule-num { min-width: 22px; height: 22px; border-radius: 50%; background: var(--blue-lt); color: var(--blue); font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 1px; border: 1px solid var(--blue-bd); }

@media (max-width: 900px) { .dashboard { grid-template-columns: 1fr 1fr; } .dashboard > .card:first-child { grid-column: 1 / -1; } }
@media (max-width: 600px) { .dashboard { grid-template-columns: 1fr; } nav { padding: 0 16px; } .nav-title { font-size: 15px; } }
</style>
</head>
<body>
  ...
  <div class="card">
    ...
    <div class="student-avatar">
      <div class="avatar-circle">
        <?php if ($photoSrc): ?>
          <img src="<?php echo $photoSrc; ?>" alt="Profile Photo"/>
        <?php else: ?>
          <svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
        <?php endif; ?>
      </div>
    ...
    <!-- Info rows — all read from freshly-synced session -->
    <div class="student-info-list">
      ...
      <div class="info-row">
        ...
        <span><span class="info-label">ID No:</span><span class="info-value"><?php echo htmlspecialchars($_SESSION['id_number'] ?? ''); ?></span></span>
      </div>
      ...
      <div class="info-row">
        ...
        <span><span class="info-label">Name:</span><span class="info-value"><?php echo htmlspecialchars($_SESSION['fullname'] ?? ''); ?></span></span>
      </div>
      ...
      <div class="info-row">
        ...
        <span><span class="info-label">Course:</span><span class="info-value"><?php echo htmlspecialchars($_SESSION['course'] ?? ''); ?></span></span>
      </div>
      ...
      <div class="info-row">
        ...
        <span><span class="info-label">Year:</span><span class="info-value"><?php echo htmlspecialchars($_SESSION['year_level'] ?? ''); ?></span></span>
      </div>
      ...
      <div class="info-row">
        ...
        <span><span class="info-label">Email:</span><span class="info-value"><?php echo htmlspecialchars($_SESSION['email'] ?? ''); ?></span></span>
      </div>
      ...
      <div class="info-row">
        ...
        <span><span class="info-label">Address:</span><span class="info-value"><?php echo htmlspecialchars($_SESSION['address'] ?? ''); ?></span></span>
      </div>
      ...
      <div class="info-row">
        ...
        <span><span class="info-label">Session:</span><span class="info-value"><?php echo htmlspecialchars($_SESSION['session'] ?? ''); ?></span></span>
      </div>
    ...
    <div class="ann-scroll">
      <?php if (!empty($announcements)): ?>
        <?php foreach ($announcements as $ann): ?>
          <div class="ann-item">
            <div class="ann-meta">
              <?php echo htmlspecialchars($ann['admin_name'] ?? 'CCS Admin'); ?>
              &nbsp;|&nbsp;
              <?php echo date('M d, Y', strtotime($ann['created_at'])); ?>
            </div>
            <?php if (!empty($ann['content'])): ?>
              <div class="ann-bubble"><?php echo htmlspecialchars($ann['content']); ?></div>
            <?php else: ?>
  ...
  <div class="card">
    ...
    <div class="rules-scroll">
      ...
      <div class="rules-list">
        ...
        <div class="rule-item"><span class="rule-num">6</span><span>Any damage to laboratory equipment due to negligence or misuse shall be the responsibility of the student concerned.</span></div>
        ...
        <div class="rule-item"><span class="rule-num">8</span><span>Students must log out and properly shut down computers after use. Lights and air conditioning units must be turned off when leaving.</span></div>
      </div>
    </div>
  </div>
